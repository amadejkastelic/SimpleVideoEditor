        ...
        for (auto && instruction : instructions) {
            if (instruction.IsMap() && instruction["part"].IsDefined()) {
                ...
                if (boost::iequals(type, "video")) {
                    ...
                    string blackImagePath = QSettings().fileName().left(QSettings().fileName().lastIndexOf("/") + 1).append("black.jpg").toStdString();
                    uint length = parseInt(instruction["part"]["length"].as<string>(), variables);
                    AudioFile *audio = nullptr;
                    if (instruction["part"]["audio"].IsMap()) {
                        string audioPath = parseString(instruction["part"]["audio"]["path"]
                    ...
                    string value = parseString(instruction["part"]["value"].as<string>(), variables);
                    string position = parseString(instruction["part"]["position"].as<string>(), variables);
                    string font = parseString(instruction["part"]["font"].as<string>(), variables);
                    string color = parseString(instruction["part"]["color"].as<string>(), variables);
                    int size = parseInt(instruction["part"]["size"].as<string>(), variables);
                    text.push_back(new Text(value, size, font, color, position));
                    result.push_back(new ImageFile(path, length, audio, text, scale, "none"));
                }
